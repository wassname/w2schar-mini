...
keep_target = len(kept) + requested_keeps
...
for round in 0..max_rounds:
    ...
    if n_keeps >= keep_target or n_drops >= MAX_DROPS:
        break
    if n_keeps + n_drops >= max_rounds:                   # safety cap
        break
```
...
Each step makes four steered forwards: cho and rej at both `c = +1` and `c = -1`.
When KL is enabled it also makes two no-gradient reference forwards at `c = 0`.
`C` is fixed at `1.0` so train-c and inference-c coincide; the earlier per-step
...
| `valid_json`    | all four IID/OOD × single/multiturn questions still close parseable `{"ans": bool}` | `≥ 1.0 · base` (strict) |
...
def c_scan(model, lora, init_c, sign):
    ...
    for i in 0..MAX_QUESTIONS:
        ...
        c *= STEP
        if c < C_MIN:
            warn("no coherent c above C_MIN; clamped")
            c = C_MIN
            break
