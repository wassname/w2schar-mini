...
def _petrov_answer(round_dir: Path, phase: str) -> str | None:
    """First assistant reply on the first probe (surveillance_1p, the in-role
    bulk-surveillance action), or None. Name kept for the JS dataset keys; the id
    ...
    for probe in d.get("probes", []):
        if probe.get("id") != "surveillance_1p":
            continue
