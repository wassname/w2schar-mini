...
::: {.tldr}
TL;DR: an early yes, a weak model can steer a stronger model's character. A weak 9B teacher steered a much stronger 27B student (about three times its size) toward a written character spec, with no labels. Across two runs the student cares more and defers less to authority, the direction we aimed for. Small models so far, but it looks like it could scale up.
:::

## Contributions

- Iterated weight steering composes: a single parameterized weight-steering adapter (low-rank PiSSA, with strict contrastive-pair filtering and a coherence-calibration pass) can be trained and composed round after round to shift a model's character cumulatively.
...
For me this is an update on a possible useful tool for alignment: a weak model reaching up to steer a stronger one, in weight space, on the model's own completions, with only a written character spec as the target. It has properties I like:
