...
::: {.tldr}
TL;DR: an early yes, a weak model can steer a stronger model's character. A small 9B teacher steered a 27B student (three times its parameter count) toward a written character spec, with no human labels. Across two runs the student cares more and defers less to authority, the direction we aimed for. Small models so far. (Caveat: the teacher is smaller but newer and reasoning-capable, so "weak" here is by size, not clearly by capability; see [Limitations](#limitations).)
:::
...
<!-- TODO(rerun): "much weaker / stronger" overclaims capability (9B teacher is newer + reasoning-capable). Holds on params, not benchmark. Re-word once the weaker-teacher rerun lands; see Limitations. -->
- Weak-as-curator weak-to-strong: evidence that a smaller model (9B) can steer a larger one's (27B) moral character through that loop, with the weak model only selecting the axis and judging keep/drop while the strong student generates the contrastive poles, distinct from prior weak-to-strong steering, which extracts the steering vector from the weak model.

## Method

A small teacher model (`qwen/qwen3.5-9b` [@qwen35omni]) iteratively steers a larger student (`google/gemma-2-27b-it` [@gemma2]) toward the moral character described in [Forethought's essay on AI character](https://www.forethought.org/research/the-importance-of-ai-character) [@forethought_character], using weight steering (the small teacher adjusting the larger student's compass, pictured above). Both runs nudged the student the same way, to act for whoever is affected rather than take the easy path or follow a bad order.

| Model | Role | Released | Params | Reasoning | Benchmark |
|---|---|---|---|---|---|
| `qwen/qwen3.5-9b` | teacher | Mar 2026 | 9B | yes (thinking) | MMLU-Pro 82.5 [@qwen35omni] |
| `google/gemma-2-27b-it` | student | Jul 2024 | 27B | no | MMLU 75.2 (5-shot) [@gemma2] |

The two scores are not the same suite (MMLU-Pro is harder than MMLU, so the numbers understate the gap), but the picture is clear: the teacher has 3x fewer parameters yet is the newer, reasoning-capable model. So "weak" here means smaller, not clearly weaker in capability. A cleaner weak-to-strong claim needs a teacher that is weaker on a common benchmark too (see [Limitations](#limitations)).

<!-- TODO(user): replace the two mismatched scores with one common cross-model index (Artificial Analysis Intelligence Index) for both models, so the benchmark column is comparable. -->
<!-- TODO(rerun): pick teacher genuinely weaker on a shared benchmark (e.g. older/smaller dense model) so weak->strong holds on capability, not just param count. Re-fill this table with the AA index for both. -->
...
- Weak-to-strong is by parameter count, not clearly by capability. The teacher (`qwen3.5-9b`, Mar 2026, reasoning-capable) is smaller than the student (`gemma-2-27b-it`, Jul 2024, no reasoning) but probably not weaker on a common benchmark, so the strong form of the claim is not yet shown. The fix is a rerun with a teacher that is weaker on a shared benchmark too.
