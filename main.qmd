...
feasibility -- weak teacher curates 1-2 genuine steers; iteration then saturates,
...
| Model | Role | Released | Params | AAII |
|---|---|---|---|---|
| `Qwen/Qwen3.6-27B` | student | Apr 2026 | 27B | 29 |
| `qwen/qwen3.5-9b` | teacher | Mar 2026 | 9B | 25 (20 without reasoning) |

Both models sit on the same [Artificial Analysis Intelligence Index](https://artificialanalysis.ai/) (AAII), a 0-100 composite of nine evaluations, so the comparison is like-for-like (values and sourcing in [`docs/2026-06-25_artificialanalysis_open_weights_index.md`](docs/2026-06-25_artificialanalysis_open_weights_index.md)). The student leads the teacher by +4 with the teacher in reasoning mode and +9 with reasoning off, a genuine if modest weak-to-strong gap, and the best one currently runnable (the wider-gap students were either gated on HuggingFace or could not be steered to embody the negative pole). They are the same model family one generation, about two months, apart, which I treat as the realistic deployment condition (a lab aligning its next model with its current one) rather than a confound to remove (see [Limitations](#limitations)).

Each round teaches the student one behaviour. The teacher is the weak model, and its job is three judgements: decide the steering personas, filter the student's own generations down to the ones that act out the target behaviour, and accept or reject whether the resulting adapter is behaviour worth saving into the student permanently. Because the teacher is weak it is held to these judgements rather than the harder generation, which the strong student does.
...
<!-- TODO(rerun): every number, plot, and interview from here through the Appendix is from the SUPERSEDED gemma-2-27b runs (0622, 0623). Refill from the regenerating Qwen3.6-27B runs: tinymfv care/authority deltas below, the two run cards (slugs + kept/rounds), and the round-3 tool-call trace + before/after interviews in the Appendix. Do NOT ship with the gemma-2-27b content under a Qwen3.6-27B method. -->
...
<!-- TODO(inline-plots): reader wants the interactive plotly panels inline here, not only as click-through links. The static scatter.svg is inline already; embed the plotly (iframe/quarto include of index.html, or export the key panel) so the main text carries the figure. -->
...
<!-- TODO(rerun): this whole appendix trace is from the gemma-2-27b run 0622. Re-trace one round of the regenerating Qwen3.6-27B run (slug, round, all six source paths below). -->
