...
Feeds a PAST run's REAL student-generated pair poles to the REAL weak
qwen-9b teacher with the live rating brief, then measures how many pairs
...
from csm.pipeline import _gen_pair_summary
from csm.prompts import TOOL_RATE_PAIRS
...
def rate_batch(axis_ctx: str, batch_items: list[dict], reverse: bool) -> dict[str, dict]:
    ...
    summary = _gen_pair_summary({"items": batch_items})
    order = "REVERSE order (last pair first)" if reverse else "forward order"
    sys_msg = (
        "You are a weak teacher curating contrastive training pairs for a stronger "
        "student. " + axis_ctx + "\n\n" + TOOL_RATE_PAIRS +
        f"\n\nRate the pairs below in {order}. Return ONLY a JSON array of "
...
def main(round_dir: str):
    cands = json.loads(open(f"{round_dir}/gen_pairs.json").read())
    ...
    c0 = next(c for item in cands["items"] for c in item["pairs"] if c["kept"])
    axis_ctx = (
    ...
    # flatten kept pairs, keep parent-item context, group into scenario-coherent batches
    kept = []
    for item in cands["items"]:
        for c in item["pairs"]:
            if c["kept"]:
                kept.append((item, c))
    print(f"axis={axis}  kept pairs={len(kept)}  floor={FLOOR}")
    ...
    for reverse in (False, True):
        ...
        for i in range(0, len(seq), BATCH):
            chunk = seq[i:i + BATCH]
            # regroup chunk into items so _gen_pair_summary renders scenario headers
            by_item = {}
            for item, c in chunk:
    ...
    print(f"\nrated {len(rows)}/{len(kept)} pairs (avg of 2 passes)")
