...
TL;DR: Can a weak model steer a stronger model's moral character? We release code and two runs that provide early evidence that it can work and is a tractable alignment technique.
...
- Iterated weight steering: each round's adapter is baked into the model before the next round trains. The harness calibrates each adapter's strength until outputs stay coherent, and keeps it only if it leads to intended behavioural changes (about 75% of the time, falling as the behaviour saturates).
- Steering as a tool for the weak teacher: we show examples that a weaker model can steer a stronger one's moral character using weight steering in a limited agentic harness.
...
This is a genuine, if modest, weak-to-strong capability gap. We treat same-lineage succession, where one model helps develop and align its stronger, larger, and newer successor by writing evals and RL environments, as a plausible deployment condition rather than an established empirical claim.
...
We use [Weight Steering](https://github.com/safety-research/weight-steering), which trains separate adapters on positive and negative completions, then uses the difference between their weight updates as the steering direction [@fierro2025weightsteering]. We [changed it](https://github.com/wassname/cwsteer/) to train one signed adapter on both poles, using a contrastive margin objective and a KL penalty to the unsteered model. We also filter the training pairs more strictly and calibrate the steering coefficient down until held-out generations remain coherent.

### More

This method section was intentionally brief, but the full details are open and runnable in the open-source [code](https://github.com/wassname/w2schar-mini). We suggest pointing an agent toward the code and asking it questions.
...
We think this work shows that this novel alignment direction is possible, and worth pursuing.
...
There are many other related pieces of work that are left out. We've tried to cite one notable work from each subfield; we encourage you to read the related work section of each to find a full list of related work.
